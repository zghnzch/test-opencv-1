...
public class OpenCv {
	...
	public static void main(String[] args) {
		String sheet = "E:\\opencv\\A4.jpg";
		//A4 二值化膨胀后生成的图片路径
		String results = "E:\\opencv\\A4result.jpg";
		String msg = rowsAndCols(sheet, results);
		System.out.println(msg);
	}
	private static void canny(String oriImg, String dstImg, int threshold) {
		//装载图片
		...
		Imgcodecs.imwrite("E:\\opencv\\A4result2.jpg", srcImage4);
		// 确定每张答题卡的ROI区域
		Mat imagCh1 = srcImage4.submat(new Rect(200, 1065, 1930, 2210));
		//识别所有轮廓
		Vector<MatOfPoint> chapter1 = new Vector<>();
		Imgproc.findContours(imagCh1, chapter1, new Mat(), 2, 3);
		Mat result = new Mat(imagCh1.size(), CV_8U, new Scalar(255));
		Imgproc.drawContours(result, chapter1, -1, new Scalar(0), 2);
		Imgcodecs.imwrite(dstImg, result);
		//new一个 矩形集合 用来装 轮廓
		List<RectComp> rectComps = new ArrayList<>();
		for (MatOfPoint matOfPoint : chapter1) {
		...
		rectComps.sort(Comparator.comparingInt(o -> o.rm.x));
		/*-----------------------------*/
		//如果精度高，可以通过像素计算
		goTestFor0(rectComps, listenAnswer);
		/*-----------------------------*/
		// 根据 Y轴 确定被选择答案 (A\B\C\D)
		goTestFor1(rectComps, listenAnswer);
		/*-----------------------------*/
	}
	private static String rowsAndCols(String oriImg, String dstImg) {
		String msg = "";
		canny(oriImg, dstImg, 100);
	...
	private static void goTestFor0(List<RectComp> rectComps, TreeMap<Integer, String> listenAnswer) {
		for (RectComp rc : rectComps) {
			int x = rectComps.get(14).getRm().x - 16;
			int y = rectComps.get(14).getRm().y - 94;
	...
	private static void goTestFor1(List<RectComp> rectComps, TreeMap<Integer, String> listenAnswer) {
		for (RectComp rc : rectComps) {
			for (int h = 0; h < 7; h++) {
		...
		for (Map.Entry<Integer, String> integerStringEntry : listenAnswer.entrySet()) {
			Object key = ((Map.Entry) integerStringEntry).getKey();
			Object val = ((Map.Entry) integerStringEntry).getValue();
